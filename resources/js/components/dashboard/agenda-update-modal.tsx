...
export default function AgendaUpdateModal({ Open, selectedAgenda }) {
    ...
    const [start, end] = selectedAgenda.time.split('-');

    useEffect(() => {
        setOpen(Open);
        setNamaAgenda(selectedAgenda.title);
        setDate(selectedAgenda.date);
        setLokasi(selectedAgenda.location);
        setTime1(start);
        setTime2(end);
        setFoto(selectedAgenda.image);
    }, [Open, selectedAgenda]);
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        router.put(
            route('agenda.update', selectedAgenda.id),
            {
                nama_agenda,
                date,
                lokasi,
        ...
        <>
            ...
            >
                <DialogContent className="h-[90vh] max-w-md overflow-y-auto sm:max-w-[600px]">
                    <DialogHeader>
                        <DialogTitle>Edit Agenda Ponpes</DialogTitle>
                        <DialogDescription>Isi detail agenda baru. Klik Edit setelah selesai.</DialogDescription>
